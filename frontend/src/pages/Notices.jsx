...
import { Megaphone, Plus, Mail, Users, Calendar, Filter, Send } from "lucide-react";
    ...
    <div className="space-y-6" data-testid="notices-page">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-slate-500 text-sm">
            Official announcements, alerts, and college updates.
          </p>
        </div>
        {user?.role === "teacher" && (
          <button
      ...
      {showForm && user?.role === "teacher" && (
        <form onSubmit={create} className="aura-card p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            <div>
              ...
              <select
                ...
                className="mt-1 w-full rounded-xl border border-slate-200 px-4 py-2.5 outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 bg-white"
              >
          ...
          <div>
            <label className="text-xs font-semibold text-slate-600 uppercase tracking-wider">Notice Content</label>
            <textarea
              required
              rows={4}
          ...
          <div className="flex justify-end gap-2">
            ...
            <button
              ...
              className="pill bg-gradient-to-r from-indigo-600 to-purple-600 text-white flex items-center gap-2 shadow-md hover:shadow-lg transition disabled:opacity-60"
            >
              <Send className="w-4 h-4" /> {submitting ? "Broadcasting..." : "Broadcast & Email"}
            </button>
          </div>
        </form>
      )}
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <button
          ...
          className={`pill ${audienceFilter === "all" ? "bg-indigo-600 text-white" : "bg-white text-slate-700 border border-slate-200 hover:bg-slate-50"}`}
        >
          All Notices
        </button>
        <button
          onClick={() => setAudienceFilter("students")}
          className={`pill ${audienceFilter === "students" ? "bg-indigo-600 text-white" : "bg-white text-slate-700 border border-slate-200 hover:bg-slate-50"}`}
        >
          For Students
        </button>
        <button
          onClick={() => setAudienceFilter("teachers")}
          className={`pill ${audienceFilter === "teachers" ? "bg-indigo-600 text-white" : "bg-white text-slate-700 border border-slate-200 hover:bg-slate-50"}`}
        >
          For Faculty
        </button>
      </div>
        ...
        <div className="space-y-4">
            ...
            >
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-start gap-3">
                  <div className="w-10 h-10 rounded-xl bg-indigo-100 text-indigo-600 grid place-items-center flex-shrink-0 mt-0.5">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-3 text-xs text-slate-400 mt-1 flex-wrap">
                      <span className="flex items-center gap-1 font-medium text-slate-600">
                        <Users className="w-3.5 h-3.5" /> Posted by {n.author_name} ({n.author_role})
                      </span>
                      <span>•</span>
                ...
                <span className="pill bg-indigo-100 text-indigo-700 capitalize text-xs">
                  {n.audience === "all" ? "Campus-wide" : n.audience}
                </span>
              </div>
          ...
          {filtered.length === 0 && (
            <div className="text-center py-16 text-slate-500 aura-card">
              ...
              <div className="font-semibold text-slate-700">No notices posted yet</div>
              <div className="text-sm text-slate-400 mt-1">Check back later for campus announcements.</div>
            </div>
          )}
        </div>
      )}
